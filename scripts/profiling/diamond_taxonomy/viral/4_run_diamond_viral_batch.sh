...
DB="${PROJECT}/databases/diamond/refseq_viral_proteins/refseq_viral_proteins"
OUT_DIR="${PROJECT}/results/diamond/viral"

mkdir -p "${OUT_DIR}"

# Require a list file to be passed in
if [[ $# -lt 1 ]]; then
  echo "[ERROR] Usage: sbatch run_diamond_viral_batch.sh <sample_list_file>"
  exit 1
fi
...
[[ -f "${DB}.dmnd" ]] || { echo "[ERROR] DIAMOND DB not found: ${DB}.dmnd"; exit 1; }
...
while IFS= read -r FASTQ; do
  ...
  OUT_TSV="${SDIR}/${SAMPLE}_viral_diamond.tsv"
